...
import org.example.voucherissuance.controller.voucher.request.VoucherPublishV3Request;
import org.example.voucherissuance.controller.voucher.request.VoucherUseV2Request;
import org.example.voucherissuance.controller.voucher.response.*;
...
public class VoucherController {
    ...
    public VoucherPublishResponse publish(@RequestBody final VoucherPublishRequest request) {
        final String publishedVoucherCode = voucherService.publishV1(LocalDate.now(), LocalDate.now().plusDays(1830L), request.amountType());
    ...
    public VoucherDisableV2Response disableV2(@RequestBody final VoucherUseV2Request request) {
        final String orderId = UUID.randomUUID().toString().toUpperCase().replaceAll("-", "");
        voucherService.disableVoucherV2(new RequestContext(request.requesterType(), request.requesterId()), request.code());
